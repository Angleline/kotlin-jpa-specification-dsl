...
fun <T> where(makePredicate: CriteriaBuilder.(Root<T>) -> Predicate): Specification<T> =
        where<T> { root, _, criteriaBuilder -> criteriaBuilder.makePredicate(root) }!!
...
inline fun <reified T> and(specs: Iterable<Specification<T>?>): Specification<T> {
    return combineSpecifications(specs, Specification<T>::and)
}
...
inline fun <reified T> or(specs: Iterable<Specification<T>?>): Specification<T> {
    return combineSpecifications(specs, Specification<T>::or)
}

// Not
operator fun <T> Specification<T>.not(): Specification<T> = not(this)
...
// Empty Specification
inline fun <reified T> emptySpecification(): Specification<T> = where<T>(null)!!
